  ...
  },
  {
    ...
    test:q => {
      ...
      return /\b(should i|do i|would you|shall i|can i|is it worth|worth it to|when (do|should) i|what (do|should) i)\b/i.test(q)
        ...
        || /\b(strike|expiration|expiry|entry|exit|stop loss|position siz|how many contracts)\b|\blooks?\s+like\s+(a\s+|an\s+)?(long|short|buy|sell|entry|play|setup|trade)\b|\ba\s+(good|solid|clean|decent)\s+(long|short|buy|entry|play|setup|trade)\b|\b(worth|ok|okay)\s+(a\s+)?(long|short|shot|play)\b/i.test(q);
    },
    shape:[
