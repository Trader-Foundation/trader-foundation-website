...
check(BANK.length === 78, "bank has 78 questions, got " + BANK.length);
check(BANK.filter(q => q.part === 1).length === 52, "52 in part 1, got " + BANK.filter(q => q.part === 1).length);
check(BANK.filter(q => q.part === 2).length === 26, "26 in part 2, got " + BANK.filter(q => q.part === 2).length);
...
check(mc === 70, "70 mc questions, got " + mc);
check(tf === 8, "8 tf questions, got " + tf);
check(distractors === 210, "210 distractors, got " + distractors);
...
check(tracks.setting === 24, "24 setting-call questions, got " + tracks.setting);
check(tracks.strategy === 26, "26 strategy-call questions, got " + tracks.strategy);
check(tracks.product + tracks.setting + tracks.strategy === 78, "every question has a track");
...
check(examTotal("setter") === 50, "setter test is 50 questions (2 retired), got " + examTotal("setter"));
check(examTotal("ec") === 54, "EC test is 54 questions, got " + examTotal("ec"));
check(examPassMark("setter") === 40, "setter passes at 40, got " + examPassMark("setter"));
check(examPassMark("ec") === 44, "EC passes at 44, got " + examPassMark("ec"));
...
check(decodeBi({bn: 78}, 77) === 77 && decodeBi({bn: 78}, 78) === null, "bn-tagged attempts decode straight through");
check(decodeBi({bn: 69}, 68) === 68 && decodeBi({bn: 69}, 69) === null, "the 69-bank generation still decodes correctly");
